    ...
    if($number > 0) {
        for($i = 0; $i < $number; $i++) {
            ...
            if(empty($_POST["invoice"])) {
                echo "Invoice field is empty. You have to fill it out.";
                exit;
            }
            if(empty($_POST["invoice_date"])) {
                echo "Invoice date field is empty. You have to fill it out.";
                exit;
            }
            if(empty($_POST["ref_no"])) {
                echo "Reference number field is empty. You have to fill it out.";
                exit;
            }
            if(empty($_POST["part_noO"][$i])) {
                echo "Ordered part field is empty. You have to fill it out.";
                exit;
            }
            if(empty($_POST["part_noR"][$i])) {
                echo "Received part field is empty. You have to fill it out.";
                exit;
            }
            if(empty($_POST["part_description"][$i])) {
                echo "Part description field is empty. You have to fill it out.";
                exit;
            }
            if(empty($_POST["quantity"][$i])) {
                echo "Quantity field is empty. You have to fill it out.";
                exit;
            }
            else {
                ...
                if ($qty > 1) {
                    for($i = 0; $i < $qty; $i++) {
                        $qtyNew = "1";
                        //Unosi u tabelu BLANK, podatke koje smo popunili u formi
                        ...
                        $stmt->bind_param('sssis', $_POST["part_noO"][$i], $_POST["part_noR"][$i], $_POST["part_description"][$i], $qtyNew, $date_short);
                        $stmt->execute();
                        
                        $req_id = mysqli_insert_id($connect); // Uzima zadnji insertovan ID iz baze
            
                        //Unosi u tabelu INVOICES, podatke koje smo popunili u formi
                        ...
                        $stmt2->bind_param('sssissssssssi', $_POST["part_noO"][$i], $_POST["part_noR"][$i], $_POST["part_description"][$i], $qtyNew, $_POST["invoice"], $_POST["invoice_date"], $_POST["ref_no"] , $_POST["price"][$i], $_POST["currency"][$i], $_POST["note"][$i], $date_long, $date_short, $req_id);
                        $stmt2->execute();
                        echo ("Kolicina veca od 1");
                    }
                }
                else {
                    //Unosi u tabelu BLANK, podatke koje smo popunili u formi
